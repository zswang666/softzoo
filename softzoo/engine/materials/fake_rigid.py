...
def get_material_model(solver: Any):
    ...
    # Get maximal coefficient of restitution for all particle group (only used in FakeRigid material)
    max_particle_group_id = max(solver.particle_group_info.keys()) + 1
    solver.max_coef_restitution = ti.field(dtype=F_DTYPE)
    ti.root.dynamic(ti.i, max(2, max_particle_group_id)).place(solver.max_coef_restitution) # cannot instantiate dynamic S-node with shape (1,)
    for k, v in solver.particle_group_info.items():
        solver.max_coef_restitution[k] = v.max_coef_restitution
    ...
    def p2g(
        ...
        # Get Lame parameters
        h = ti.cast(5., F_DTYPE) # fake rigid; harder
        mu = mu_[p] * h
        lambd = lambd_[p] * h
