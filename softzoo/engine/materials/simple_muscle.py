...
def get_material_model(solver: Any):
    ...
    if use_actuator_to_specify_muscle_direction:
        ...
        for k, v in solver.particle_group_info.items():
            if v.muscle_direction is None:
                A = np.zeros((solver.dim, solver.dim))
            else:
                A = np.diag(v.muscle_direction)
            solver.muscle_direction[k] = A.astype(f_dtype_np).tolist()
    ...
    def p2g_corotated(
        ...
        A = solver.get_muscle_direction_matrix(p, id)
        A *= actuation[s, p] * muscle_stiffness
        stress += new_F @ A @ new_F.transpose()
